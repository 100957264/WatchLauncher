...
import com.fise.xiaoyu.protobuf.IMBaseDefine.MsgType;
...
public class MsgAnalyzeEngine {
    ...
    public static MessageEntity analyzeMessage(IMBaseDefine.MsgInfo msgInfo) {
       ...
       if(msgInfo.getMsgType() == MsgType.MSG_TYPE_SINGLE_NOTICE)
       {
    	   ...
    	   desMessage = new String(Security.getInstance().DecryptMsg(msgInfo.getMsgData().toStringUtf8()));
    	   messageEntity.setContent(desMessage);
       }
       ...
       if(!TextUtils.isEmpty(desMessage)){
           ...
           if(msgList.size()>1){
             ...
             // return TextMessage.parseFromNet(messageEntity);
        	   ...
        	   if(msgInfo.getMsgType() == MsgType.MSG_TYPE_SINGLE_NOTICE){
        		   return NoticeMessage.parseFromNet(messageEntity); 
        	   }else if(msgInfo.getMsgType() == MsgType.MSG_TYPE_VIDEO_CALL
                       ||msgInfo.getMsgType() == MsgType.MSG_TYPE_VIDEO_ANSWER
                       ||msgInfo.getMsgType() == MsgType.MSG_TYPE_VIDEO_CLOSE){
                   return OnLineVedioMessage.parseFromNet(messageEntity);
