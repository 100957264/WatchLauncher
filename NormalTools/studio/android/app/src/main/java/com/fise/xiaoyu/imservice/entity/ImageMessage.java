...
public class ImageMessage extends MessageEntity implements Serializable {
    ...
    public static ImageMessage parseFromNet(MessageEntity entity) throws JSONException {
       ...
       // String imageUrl = strContent.substring(MessageConstant.IMAGE_MSG_START.length());
        String imageUrl = strContent;//imageUrl.substring(0,imageUrl.indexOf(MessageConstant.IMAGE_MSG_END));
...
//        }
    ...
    public static ImageMessage parseFromDB(MessageEntity entity)  {
        ...
        try {
            ...
            int loadStatus = MessageConstant.IMAGE_UNLOAD;
            if(!extraContent.isNull("loadStatus")){
               // imageMessage.setUrl(extraContent.getString("url"));
                loadStatus = extraContent.getInt("loadStatus");
            }
    ...
    public static ImageMessage buildForSend(String takePhotoSavePath,UserEntity fromUser,PeerEntity peerEntity){
        ...
        imageMessage.setMessageTime(""+nowTime);
    ...
    public static ImageMessage buildForSend(String takePhotoSavePath,UserEntity fromUser,int peerId){
        ImageMessage imageMessage = new ImageMessage();
        int nowTime = (int) (System.currentTimeMillis() / 1000);
        imageMessage.setFromId(fromUser.getPeerId());
        imageMessage.setToId(peerId);
        imageMessage.setUpdated(nowTime);
        imageMessage.setCreated(nowTime);
        imageMessage.setDisplayType(DBConstant.SHOW_IMAGE_TYPE);
        imageMessage.setPath(takePhotoSavePath); 
        
        int msgType  = DBConstant.MSG_TYPE_SINGLE_IMAGE;
        imageMessage.setMsgType(msgType);
    ...
    //命令发送图片数据。。。
    /*
     * msgType 为MSG_TYPE_GROUP_AUTH_IMAGE 或者MSG_TYPE_SINGLE_AUTH_IMAGE
     */
    public static ImageMessage buildForSend(String takePhotoSavePath,UserEntity fromUser,PeerEntity peerEntity,String name){
        ImageMessage imageMessage = new ImageMessage();
        int nowTime = (int) (System.currentTimeMillis() / 1000);
        imageMessage.setFromId(fromUser.getPeerId());
        imageMessage.setToId(peerEntity.getPeerId());
        ...
        imageMessage.setPath(takePhotoSavePath);
        int peerType = peerEntity.getType();
        int msgType = peerType == DBConstant.SESSION_TYPE_GROUP ? DBConstant.MSG_TYPE_GROUP_AUTH_IMAGE
                : DBConstant.MSG_TYPE_SINGLE_AUTH_IMAGE;
        imageMessage.setMsgType(msgType);
...
//                + url + MessageConstant.IMAGE_MSG_END;
    ...
    public String getUrl() {
        return  SystemConfigSp.instance().getStrConfig(
				SystemConfigSp.SysCfgDimension.MSFSSERVER) + url;
    }
