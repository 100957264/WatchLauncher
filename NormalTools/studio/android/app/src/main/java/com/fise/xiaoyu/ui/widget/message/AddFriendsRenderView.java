...
/**
 * AddFriendsRenderView
 *
 * 样式根据mine 与other不同可以分成两个
 */
public class AddFriendsRenderView extends  BaseMsgRenderView {
    /** 文字消息体 */
    private TextView messageContent;

    public static AddFriendsRenderView inflater(Context context,ViewGroup viewGroup,boolean isMine){
        int resource = R.layout.tt_other_friends_message_item;

        AddFriendsRenderView  textRenderView = (AddFriendsRenderView) LayoutInflater.from(context).inflate(resource, viewGroup, false);
        textRenderView.setMine(isMine);
        textRenderView.setParentView(viewGroup);
        return textRenderView;
    }

    public AddFriendsRenderView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void render(MessageEntity messageEntity, UserEntity userEntity,Context context,PeerEntity peerEntity) {
        ...
        AddFriendsMessage textMessage = (AddFriendsMessage) messageEntity;
