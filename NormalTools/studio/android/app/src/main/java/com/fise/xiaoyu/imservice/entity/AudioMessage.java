...
public class AudioMessage extends MessageEntity implements Serializable {
    ...
    public static AudioMessage buildForSend(float audioLen, String audioSavePath, UserEntity fromUser, PeerEntity peerEntity) {
        ...
        audioMessage.setMessageTime("");

        return audioMessage;
    }


    public static AudioMessage buildForSend(float audioLen, String audioSavePath, UserEntity fromUser, PeerEntity peerEntity, String name) {
        int tLen = (int) (audioLen + 0.5);
        tLen = tLen < 1 ? 1 : tLen;
        if (tLen < audioLen) {
            ++tLen;
        ...
        audioMessage.setUpdated(nowTime);
        int peerType = peerEntity.getType();
        int msgType = peerType == DBConstant.SESSION_TYPE_GROUP ? DBConstant.MSG_TYPE_GROUP_AUTH_SOUND :
                DBConstant.MSG_TYPE_SINGLE_AUTH_SOUND;
        audioMessage.setMsgType(msgType);
