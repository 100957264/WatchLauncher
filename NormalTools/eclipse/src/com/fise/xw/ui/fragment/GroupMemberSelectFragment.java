...
public class GroupMemberSelectFragment extends MainFragment implements
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		isSelect = intent.getBooleanExtra(
				IntentConstant.KEY_SESSION_SELETC_GROUP, false);
	...
	private Set<Integer> getAlreadyCheckList() {
		...
		if (peerEntity == null) {
			Toast.makeText(getActivity(), getString(R.string.error_group_info),
	...
	private void initContactList(final Set<Integer> alreadyList) {
		...
		if (groupType == DBConstant.SESSION_TYPE_GROUP_WEI) {
			List<UserEntity> contactList = imService.getContactManager()
					.getContactWeiSelectList();
			adapter.setAllUserList(contactList);
		} else if (groupType == DBConstant.SESSION_TYPE_SINGLE) {
			List<UserEntity> contactList = imService.getContactManager()
	...
	private void initRes() {
		...
		select_group_member.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				IMUIHelper.openSelectGroupActivity(
						GroupMemberSelectFragment.this.getActivity(), false);
		...
		if (isSelect) {
			...
			group_layout_line.setVisibility(View.GONE);
		} else {
