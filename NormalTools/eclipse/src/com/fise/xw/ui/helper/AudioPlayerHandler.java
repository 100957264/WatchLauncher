...
public class AudioPlayerHandler{
    ...
    public  void setAudioMode(int mode,Context ctx) {
        ...
        SharedPreferences   user = ctx.getSharedPreferences("SP", Activity.MODE_PRIVATE);
    ...
    //语音播放的模式
    public  void setAudioMode(int mode,Context ctx,boolean speaker) {
        if (mode != AudioManager.MODE_NORMAL && mode != AudioManager.MODE_IN_CALL) {
            return;
        }
        AudioManager audioManager = (AudioManager) ctx.getSystemService(Context.AUDIO_SERVICE);
       // audioManager.setSpeakerphoneOn(speaker);//打开扬声器
      //  audioManager.setMode(mode);
