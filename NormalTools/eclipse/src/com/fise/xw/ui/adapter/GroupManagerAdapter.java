...
public class GroupManagerAdapter extends BaseAdapter {
	...
	public GroupManagerAdapter(Context c, IMService imService,
			PeerEntity peerEntity, GroupNickEntity entity) {
		memberList.clear();
		this.context = c;
	...
	private void setGroupData(GroupEntity entity) {
		...
		case DBConstant.GROUP_TYPE_WEI_TEMP: {
			if (loginId == entity.getCreatorId()) {
				showMinusTag = true;
				showPlusTag = true;
			} else {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (position >= 0 && memberList.size() > position) {
			...
			String ShowName;
			if (isGroupNick
					&& (peerEntity.getType() == DBConstant.SESSION_TYPE_GROUP)) {

				
				if(userEntity.getIsFriend() == DBConstant.FRIENDS_TYPE_YES
				  ||userEntity.getIsFriend() == DBConstant.FRIENDS_TYPE_WEI){
			...
			setHolder(holder, position, null, R.drawable.liaotianxinx_add_normal, "",
					null); //guanweile  add_chat_details
			holder.imageView.setOnClickListener(new View.OnClickListener() {
				@Override
						...
						if (groupEntity.getSave() != DBConstant.GROUP_MEMBER_STATUS_EXIT) {
							...
							if (groupEntity.getGroupType() == DBConstant.GROUP_TYPE_WEI_TEMP) {
								...
								IMUIHelper.openGroupMemberSelectActivity(context,
										peerEntity.getSessionKey(),true,DBConstant.SESSION_TYPE_GROUP_WEI);
							}else{
								IMUIHelper.openGroupMemberSelectActivity(context,
										peerEntity.getSessionKey(),true,DBConstant.SESSION_TYPE_GROUP);
							}
						}
					} else {
						IMUIHelper.openGroupMemberSelectActivity(context,
								peerEntity.getSessionKey(),true,DBConstant.SESSION_TYPE_SINGLE);
					}
			...
			setHolder(holder, position, null,
					R.drawable.liaotianxinx_subtract_normal, "", null);
