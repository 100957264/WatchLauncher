...
public class PassActivityVerify extends TTBaseActivity {
			...
			public void onClick(View view) {
				...
				if ((!mPassword.getText().toString().equals(""))
					...
					if (!(mPassword.getText().toString().equals(mPassZaiword
							.getText().toString()))) {
						Toast.makeText(PassActivityVerify.this, "密码不一样 请重新输入",
					...
					if (result) {
						Toast.makeText(PassActivityVerify.this, "密码不能是纯数字",
								Toast.LENGTH_SHORT).show();
						return;
					}
					...
					if (resultHan) {
						Toast.makeText(PassActivityVerify.this, "密码不能是汉字",
								Toast.LENGTH_SHORT).show();
						return;
					}

					// boolean resultZi =
					// isChar(mPassword.getText().toString());
					// if (resultZi) {
					// Toast.makeText(PassActivityVerify.this, "密码不能是纯字母",
					// Toast.LENGTH_SHORT).show();
					// return;
					// }
					...
					if (mPassword.getText().toString().indexOf(" ") >= 0) {
						// 含有空格
						Toast.makeText(PassActivityVerify.this, "密码不能有空格",
								Toast.LENGTH_SHORT).show();
						return;
					}
					...
					if (mPassword.getText().toString().length() < DBConstant.PASS_LENGTH) {
						// 含有空格
						Toast.makeText(PassActivityVerify.this, "密码不能少于6位",
								Toast.LENGTH_SHORT).show();
						return;
					}
					...
					Toast.makeText(PassActivityVerify.this, "请输入正确的密码",
							Toast.LENGTH_SHORT).show();
				}
			}
		});
	...
	public void onEventMainThread(UserInfoEvent event) {
		...
		case USER_MODIFY_PASS_FAIL:
			...
			Toast.makeText(PassActivityVerify.this,
					"" + imService.getUserActionManager().getModifyPass(),
					Toast.LENGTH_SHORT).show();
			break;
		case USER_MODIFY_PASS_SUCCESS:
			Toast.makeText(PassActivityVerify.this, "修改密码成功",
					Toast.LENGTH_SHORT).show();
			PassActivityVerify.this.finish();
			break;
		}
