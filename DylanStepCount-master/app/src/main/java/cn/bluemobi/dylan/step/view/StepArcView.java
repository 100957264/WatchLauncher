...
public class StepArcView extends View {
     ...
     * 圆弧的宽度
     */
    private float borderWidth = dipToPx(30);

    private float backgroundBoderWidth = dipToPx(48);
    /**
     * 画步数的数值的字体大小
     */
     ...
     * 终点对应的角度和起始点对应的角度的夹角
     */
    private float angleLength = 300;
    /**
     * 所要绘制的当前步数的红色圆弧终点到起点的夹角
     */
    private float currentAngleLength = 0;
    private float actualAngleLength = 0;
    /**
     * 动画时长
     */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawArcBackground(canvas, rectF);
        /**【第一步】绘制整体的黄色圆弧*/
        ...
        drawTextTargetString(canvas,centerX);
        drawTextPercentString(canvas,centerX);
        drawTextTergetValueString(canvas,centerX);
    }
    ...
    private void drawArcBackground(Canvas canvas, RectF rectF) {
        Paint paint = new Paint();
        /** 默认画笔颜色，黄色 */
        ...
        paint.setAntiAlias(true);
        /**设置画笔宽度*/
        paint.setStrokeWidth(backgroundBoderWidth);
    ...
    private void drawArcRed(Canvas canvas, RectF rectF) {
        ...
        int steps = Integer.valueOf(stepNumber);
        if(steps <= 10000) {
            paintCurrent.setColor(getResources().getColor(R.color.green));//设置画笔颜色
        }else if(steps > 10000 && steps <= 20000){
            paintCurrent.setColor(getResources().getColor(R.color.color_three));//设置画笔颜色
        }else if(steps > 20000 && steps <= 30000) {
            paintCurrent.setColor(getResources().getColor(R.color.lowred));//设置画笔颜色
        }else {
            paintCurrent.setColor(getResources().getColor(R.color.purple));//设置画笔颜色
        }
    ...
    private void drawTextNumber(Canvas canvas, float centerX) {
        ...
        vTextPaint.setColor(getResources().getColor(R.color.greyWhite));
    ...
    private void drawTextStepString(Canvas canvas, float centerX) {
        Paint vTextPaint = new Paint();
        vTextPaint.setTextSize(dipToPx(28));
        vTextPaint.setTextAlign(Paint.Align.CENTER);
        vTextPaint.setAntiAlias(true);//抗锯齿功能
        vTextPaint.setColor(getResources().getColor(R.color.greyWhite));
        String stepString = "今日";
     ...
     * 4.圆环中心[目标]的文字
     */
    private void drawTextTargetString(Canvas canvas, float centerX) {
        Paint vTextPaint = new Paint();
        vTextPaint.setTextSize(dipToPx(22));
        vTextPaint.setTextAlign(Paint.Align.CENTER);
        vTextPaint.setAntiAlias(true);//抗锯齿功能
        vTextPaint.setColor(getResources().getColor(R.color.greyWhite));
        String stepString = "目标";
        Rect bounds = new Rect();
        vTextPaint.getTextBounds(stepString, 0, stepString.length(), bounds);
        canvas.drawText(stepString, centerX, getHeight() / 2 + bounds.height() + getFontHeight(numberTextSize) + dipToPx(30), vTextPaint);
    }
    /**
     * 4.圆环底部百分百的文字
     */
    private void drawTextPercentString(Canvas canvas, float centerX) {
        Paint vTextPaint = new Paint();
        vTextPaint.setTextSize(dipToPx(22));
        vTextPaint.setTextAlign(Paint.Align.CENTER);
        vTextPaint.setAntiAlias(true);//抗锯齿功能
        vTextPaint.setColor(getResources().getColor(R.color.greyWhite));
        String stepString = getPercent() +  "%";
        Rect bounds = new Rect();
        vTextPaint.getTextBounds(stepString, 0, stepString.length(), bounds);
        canvas.drawText(stepString, centerX, getHeight() / 2 + bounds.height() + getFontHeight(numberTextSize) + dipToPx(10), vTextPaint);
    }
    /**
     * 4.圆环底部10000的文字
     */
    private void drawTextTergetValueString(Canvas canvas, float centerX) {
        Paint vTextPaint = new Paint();
        vTextPaint.setTextSize(dipToPx(24));
        vTextPaint.setTextAlign(Paint.Align.CENTER);
        vTextPaint.setAntiAlias(true);//抗锯齿功能
        vTextPaint.setColor(getResources().getColor(R.color.greyWhite));
        String stepString = "10000";
        Rect bounds = new Rect();
        vTextPaint.getTextBounds(stepString, 0, stepString.length(), bounds);
        canvas.drawText(stepString, centerX, getHeight() - bounds.height(), vTextPaint);
    }
    private String getPercent(){
        double percent = actualAngleLength/angleLength;
    ...
    public void setCurrentCount(int totalStepNum, int currentCounts) {
        ...
        int actualCounts = currentCounts;
        ...
        float scale2 = (float) actualCounts/totalStepNum;
        actualAngleLength = scale2*angleLength;
        stepNumber = String.valueOf(actualCounts);
        setTextSize(actualCounts);
    }
