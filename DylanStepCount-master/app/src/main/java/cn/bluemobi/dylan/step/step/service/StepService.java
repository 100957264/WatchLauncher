...
import cn.bluemobi.dylan.step.step.accelerometer.StepDetector;
...
//                    save();
                    ...
                    Log.d("fengqing" ,"ACTION_DATE_CHANGED: current step=" + CURRENT_STEP);
                    save();
                    isNewDay();
                } else if (Intent.ACTION_TIME_CHANGED.equals(action)) {
                    //时间变化步数重置为0
                    Log.d("fengqing" ,"ACTION_TIME_CHANGED: current step=" + CURRENT_STEP);
                    isCall();
                    save();
                    isNewDay();
                } else if (Intent.ACTION_TIME_TICK.equals(action)) {//日期变化步数重置为0
                    isCall();
                    Log.d("fengqing" ,"ACTION_TIME_TICK: current step=" + CURRENT_STEP);
                    save();
                    isNewDay();
                }
            }
